...
**Sessions** track blueprint execution progress across projects. Users can resume interrupted workflows from where they left off, and Hacksmith automatically backs up project data for recovery.
...
Configure your AI assistant and analyze your project's tech stack.

```bash
hacksmith preferences setup
```

Check your current session status and project information.
...
Hacksmith uses a project-aware storage system that keeps data organized and prevents conflicts between different projects:

**Project-local storage** (`.hacksmith/` in project root):

- Stores blueprint variables, tech stack analysis, and session state
- Travels with your project and can be version controlled or shared with team members
- Automatically created when you run blueprints in a project directory

**System-level storage** (`~/.hacksmith/`):

- Stores AI agent preferences and global settings
- Maintains backups of the last 10 projects for disaster recovery
- Shared across all projects on your system

### Session Management

Sessions track your progress through blueprint execution and enable resumption of interrupted workflows:

- **Automatic detection** of project boundaries using git repositories and package.json files
- **Progress tracking** that saves your current step and captured variables
- **Resume prompts** when you restart a blueprint that was previously interrupted
- **Data backup** after successful completion to prevent loss of integration work
